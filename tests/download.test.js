...
import { download, lastModifiedAtOrigin } from '../src/download-utils.js'
...
test('should check for freshness using HEAD request', async t => {
  const lastModifiedDate = '2019-07-01T20:32:59.000Z'
  const scope = nock('https://' + maxMindHost)
    .head(maxMindPath)
    .reply(200, {}, { 'Last-Modified': lastModifiedDate })

  const serverMtime = await lastModifiedAtOrigin()
  t.deepEqual(serverMtime, new Date(lastModifiedDate))
  scope.done()
})
